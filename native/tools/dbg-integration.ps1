...
function Start-Gui() {
    ...
    $p = Start-Process -FilePath $Ctrl -PassThru
...
function Test-Section([string]$name, [scriptblock]$body) {
    ...
    try { & $body } catch {
        Write-Host "  ERROR $($_.Exception.Message)" -ForegroundColor Red; Write-Host "  STACK: $($_.ScriptStackTrace)"
...
Test-Section 'B' {
    ...
    Assert 'start-exit-0' ($r.Exit -eq 0) "exit=$($r.Exit) out=$($r.Out)"
...
Test-Section 'C' {
    $s = Headless 'status'
    Assert 'status-running' ($s.Out -like 'STATE Running*') "got: $($s.Out)"
    Assert 'not-marked-already-running' ($s.Out -match 'already-running=false') "got: $($s.Out)"
    $h = Headless 'health'
...
Test-Section 'D' {
    $r = Headless 'stop' 90
    Assert 'stop-exit-0' ($r.Exit -eq 0) "exit=$($r.Exit) out=$($r.Out)"
...
Test-Section 'E' {
    ...
    Assert 'start' ($r1.Exit -eq 0 -and $r1.Out -eq 'STARTED')
    $r2 = Headless 'restart' 180
    Assert 'restart-exit-0' ($r2.Exit -eq 0) "exit=$($r2.Exit) out=$($r2.Out)"
...
Test-Section 'F' {
    $r1 = Headless 'start' 120
    Assert 'first-start' ($r1.Exit -eq 0 -and $r1.Out -eq 'STARTED')
    $r2 = Headless 'start' 60
    Assert 'second-start-refused' ($r2.Exit -ne 0 -and $r2.Out -like 'FAILED*already-running*') "got: $($r2.Out)"
    Start-Sleep -Milliseconds 800
    $state = Get-Content $StateFile -ErrorAction SilentlyContinue
...
Test-Section 'G' {
    ...
    try {
        $r = Headless 'start' 90
        Assert 'start-refused-on-conflict' ($r.Exit -ne 0 -and $r.Out -match 'in use') "got: $($r.Out)"
        $holder.Refresh()
        Assert 'unrelated-process-not-killed' (-not $holder.HasExited)
        Assert 'mariadb-not-started' (PortFree $MariaPort)
        $s = Headless 'status'
        Assert 'status-failed' ($s.Out -like 'STATE Failed*') "got: $($s.Out)"
    } finally {
...
Test-Section 'H' {
    ...
    try {
        $r = Headless 'start' 120
        Assert 'start-fails-on-mariadb' ($r.Exit -ne 0 -and $r.Out -match 'MariaDB') "got: $($r.Out)"
...
Test-Section 'J' {
    $r = Headless 'start' 120
    Assert 'server-started' ($r.Exit -eq 0)
    $gui = Start-Gui
    ...
    if ($gui.MainWindowHandle -ne 0) {
        ...
        Assert 'gui-closes-cleanly' ($closed -and $gui.ExitCode -eq 0) "exit=$($gui.ExitCode)"
    } else {
        $gui.Kill()
...
Test-Section 'L' {
    ...
    $r = Headless 'start' 120
    Assert 'start-succeeds-despite-stale-state' ($r.Exit -eq 0 -and $r.Out -eq 'STARTED') "got: $($r.Out)"
    Start-Sleep -Milliseconds 800
    $state = Get-Content $StateFile -ErrorAction SilentlyContinue
