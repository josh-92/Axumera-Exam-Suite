<#
    student-kiosk-e2e.ps1 - Phase 4 full interactive kiosk test against the
    isolated AXE 2.0 dev runtime (Apache 8090 / MariaDB 3310). Drives the real
    ...
    Prerequisites:
      * dev runtime running on 8090/3310 (Axumera.Server.exe --headless start)
...
$Mysql = Join-Path $repo 'native\dev-runtime\runtime\mariadb\bin\mysql.exe'
...
$apacheUp = Get-NetTCPConnection -LocalPort 8090 -State Listen -ErrorAction SilentlyContinue
$mariaUp  = Get-NetTCPConnection -LocalPort 3310 -State Listen -ErrorAction SilentlyContinue
if (-not $apacheUp -or -not $mariaUp) {
    Write-Host 'FATAL: dev runtime not running (8090/3310). Start it with: Axumera.Server.exe --runtime-root native\dev-runtime --headless start' -ForegroundColor Red
    exit 99
}
...
Start-Sleep -Seconds 2
$row = & $Mysql --protocol=tcp -h 127.0.0.1 -P 3310 -u root --skip-password -N eaes_exam -e "SELECT CONCAT(ea.violation_count, '|', ea.integrity_status, '|', ea.status, '|', IFNULL(ea.score, -1), '|', IFNULL(ea.total_questions, -1)) FROM exam_attempts ea JOIN students s ON s.id = ea.student_id WHERE s.roll_number = $Roll;" 2>&1
...
$controlled = & $Mysql --protocol=tcp -h 127.0.0.1 -P 3310 -u root --skip-password -N eaes_exam -e "SELECT COUNT(*) FROM activity_log WHERE actor_identifier = '$Roll' AND action = 'integrity_violation' AND details LIKE '%controlled_exit%';" 2>&1
Assert 'exactly-one-controlled-exit' ($controlled.Trim() -eq '1') "count=$controlled"
$graded = & $Mysql --protocol=tcp -h 127.0.0.1 -P 3310 -u root --skip-password -N eaes_exam -e "SELECT COUNT(*) FROM exam_attempts ea JOIN students s ON s.id = ea.student_id WHERE s.roll_number = $Roll AND ea.score IS NOT NULL AND ea.total_questions IS NOT NULL;" 2>&1
...
$autosaved = & $Mysql --protocol=tcp -h 127.0.0.1 -P 3310 -u root --skip-password -N eaes_exam -e "SELECT IFNULL(DATE_FORMAT(ea.last_saved_at, '%Y-%m-%d %H:%i:%s'), 'NULL') FROM exam_attempts ea JOIN students s ON s.id = ea.student_id WHERE s.roll_number = $Roll;" 2>&1
