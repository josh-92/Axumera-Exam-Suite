<#
    control-panel-rehearsal.ps1 - Control Panel E2E against the isolated
    REHEARSAL runtime (Apache 8288 / MariaDB 3488) - FINAL ARCHITECTURE.
    ...
    Control Panel against the rehearsal tree, and proves:
    ...
    Usage: powershell -File tools\control-panel-rehearsal.ps1
#>
$ErrorActionPreference = 'Continue'
$repo = 'C:\Axumera-Enginnering'
$rehearsalRoot = Join-Path $repo 'rehearsal\app'
...
$accessLog = Join-Path $rehearsalRoot 'logs\apache-access.log'
$screenshot = Join-Path $repo 'native\build\control-panel-rehearsal.png'
$apachePort = 8288
$mariaPort = 3488
...
function Headless([string]$cmd, [int]$timeoutSec = 180) {
    ...
    $p = Start-Process -FilePath $serverExe -ArgumentList @('--runtime-root', $rehearsalRoot, '--headless', $cmd) `
...
function Launch-Cp() {
    $env:AXUMERA_RUNTIME_ROOT = $rehearsalRoot
...
# Server-side proof: Apache access log must show the adminlogin.php request on the rehearsal port.
...
# rehearsal ports. (A machine-wide process check false-positives when a separate
# production runtime is legitimately running, which is the post-deployment state.)
$rehearsalListeners = Get-NetTCPConnection -State Listen -LocalPort $apachePort, $mariaPort -ErrorAction SilentlyContinue
Assert 'cp2-no-rehearsal-server-started' (-not $rehearsalListeners) "rehearsal ports busy: $(($rehearsalListeners | ForEach-Object { $_.LocalPort }) -join ',')"
...
$resultsFile = Join-Path $repo 'native\build\control-panel-rehearsal-results.txt'
$summary = @()
$summary += ''
$summary += '=================================================='
$summary += 'CONTROL PANEL REHEARSAL RESULTS (FINAL ARCHITECTURE)'
$summary += '=================================================='
foreach ($line in $results) { $summary += "  $line"; Write-Host "  $line" }
if ($failures -eq 0) { $summary += 'ALL CONTROL PANEL REHEARSAL TESTS PASSED'; Write-Host 'ALL CONTROL PANEL REHEARSAL TESTS PASSED' -ForegroundColor Green }
