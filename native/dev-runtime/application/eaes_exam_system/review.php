...
/*   __________________________________________________
    ...
    |__________________________________________________|
*/
 require_once __DIR__ . '/app/bootstrap.php'; use App\Core\Csrf; use App\Repositories\AttemptRepository; use App\Repositories\ExamQuestionShuffleRepository; use App\Repositories\ExamRepository; use App\Repositories\QuestionRepository; if (isset($_SESSION['full_name'], $_SESSION['student_id'])) { goto RcRrk; } header("Location: slogin.php"); exit; RcRrk: if (!(isset($_SESSION['exam_submitted']) && $_SESSION['exam_submitted'] === true)) { goto LRMFL; } header("Location: already_taken.php"); exit; LRMFL: $WJSjm = (int) $_SESSION['student_id']; $QR3LD = ExamRepository::liveExam(); if ($QR3LD) { goto tdUZZ; } header("Location: waite.php"); exit; tdUZZ: $ugLXG = (int) $QR3LD['id']; $hkJom = AttemptRepository::find($WJSjm, $ugLXG); $gE5lz = $hkJom ? json_decode((string) $hkJom['answers'], true) ?: [] : []; $Vx_j1 = QuestionRepository::forExam($ugLXG); $mC0eI = []; foreach ($Vx_j1 as $RmthD) { if (!((int) $RmthD['is_passage'] === 1)) { goto P4Y6d; } goto dCgop; P4Y6d: $mC0eI[(int) $RmthD['question_number']] = $RmthD; dCgop: } CmdGo: $rEZEE = array_keys($mC0eI); $O30L_ = ExamQuestionShuffleRepository::find($WJSjm, $ugLXG); if (!$O30L_) { goto Cwh4x; } $ANS_h = json_decode((string) $O30L_['question_order'], true) ?: []; $KmZzB = array_values(array_filter($ANS_h, fn($Tu7Y9) => isset($mC0eI[$Tu7Y9]))); if (!$KmZzB) { goto W87fb; } $rEZEE = $KmZzB; W87fb: Cwh4x: $tRQ2E = []; $WW_Ec = 1; foreach ($rEZEE as $i30SX) { $tRQ2E[] = ['display_num' => $WW_Ec, 'db_id' => $i30SX]; $WW_Ec++; mYjXS: } haUzH: if ($tRQ2E) { goto Tb423; } $tRQ2E[] = ['display_num' => 1, 'db_id' => 1]; Tb423: ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <script>
        const metaQuestions = <?php  echo json_encode($tRQ2E); ?>;
        const savedAnswers = <?php  echo json_encode($gE5lz, JSON_UNESCAPED_UNICODE); ?>;
        const csrfToken = <?php  echo json_encode(Csrf::token()); ?>;

        // Native shell messaging (Phase 4, optional): the Axumera Student
        // client releases kiosk mode only when the server actually finalizes
        // the attempt. In a normal browser these are no-ops.
        ...
        function processFinalUpload() {
            ...
            .then(data => {
                if (data && data.status === 'success') {
                    postToNative({ type: 'exam-submitted', payload: { schemaVersion: 1, score: data.score, total: data.total } });
                }
            })
            .finally(() => {
