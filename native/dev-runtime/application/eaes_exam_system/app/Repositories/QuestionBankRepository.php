...
class QuestionBankRepository
{
    ...
    public static function facets(): array
    {
        ...
        // DISTINCT is applied to the (subject,grade,difficulty,type) row, so a
        // subject with many difficulty/type combos appears once per combo. Collapse
        // each facet to one entry per name (case/space-insensitive, keep first
        // spelling) so the dropdowns show e.g. "Biology" once, not 3 times.
        foreach (array_keys($out) as $key) {
            $out[$key] = self::uniqueFacetValues($out[$key]);
        }
        return $out;
    }

    /** Collapse a raw facet list to unique values, keeping the first-seen spelling. */
    private static function uniqueFacetValues(array $values): array
    {
        $seen = [];
        $clean = [];
        foreach ($values as $v) {
     ...
     * (including exact duplicate matches).
     *
     * Returns ['id' => int, 'created' => bool, 'duplicates' => array].
     */
    public static function save(array $data, int $actorId): array
    {
        ...
        // Duplicate gate — an exact text match (normalized) rejects the save;
        // near-matches are returned so the caller can surface them.
        $duplicates = self::gateDuplicates($n['question'], $existingId);
        ...
        if ($existingId > 0) {
            ...
            return ['id' => $existingId, 'created' => false, 'duplicates' => $duplicates];
        }
        ...
        $newId = (int) self::connection()->lastInsertId();
        return ['id' => $newId, 'created' => true, 'duplicates' => $duplicates];
    }
    ...
    /**
     * Find existing bank questions that duplicate the given text.
     *
     * Exact match on case/whitespace/punctuation normalized text —
     * deterministic, no external services required.
     *
     * Returns [{id, question, score, matched_by: exact}] sorted by score
     * desc, capped at 5. Archived questions are ignored.
     *
     * Note: the check-then-insert gate is not atomic — two simultaneous
     * saves could both pass before either inserts. For a single-admin tool
     * this is acceptable; a DB-level unique guard is not applied because it
     * would hard-reject legitimate near-identical short stems.
     */
    public static function duplicateCheck(string $question, int $excludeId = 0): array
    {
        $hits = [];
        $needle = self::normalizeText($question);

        $stmt = self::connection()->prepare(
            "SELECT id, COALESCE(question, question_text) AS question
             FROM questions
             WHERE exam_id IS NULL AND source_question_id IS NULL
               AND archived_at IS NULL AND id <> :id"
        );
        $stmt->execute(['id' => $excludeId]);
        foreach ($stmt->fetchAll() as $row) {
            if (self::normalizeText((string) ($row['question'] ?? '')) === $needle) {
                $hits[] = [
                    'id' => (int) $row['id'],
    ...
    /** Run duplicate detection and reject exact matches. Returns matches (empty = clean). */
    private static function gateDuplicates(string $question, int $excludeId = 0): array
    {
        $hits = self::duplicateCheck($question, $excludeId);
        $blocking = self::blockingHits($hits);
        if ($blocking !== []) {
            throw new InvalidArgumentException(self::duplicateMessage($blocking));
        }
        return $hits;
    }

    /** Hits that hard-block the save: exact text matches only. */
    private static function blockingHits(array $hits): array
    {
        return array_values(array_filter($hits, fn ($h) => $h['matched_by'] === 'exact'));
    }

    /** Human-readable rejection message listing the existing match(es). */
    private static function duplicateMessage(array $hits): string
    {
        $list = [];
        foreach (array_slice($hits, 0, 2) as $h) {
            $preview = mb_strlen((string) $h['question']) > 60
                ? mb_substr((string) $h['question'], 0, 60) . '…'
                : (string) $h['question'];
            $list[] = '#' . $h['id'] . ' (“' . $preview . '”)';
        }
        $more = count($hits) > 2 ? ' and ' . (count($hits) - 2) . ' more' : '';
        return 'This looks identical to an existing question in the bank (' . implode(', ', $list) . $more . ') — please review before saving.';
    }

    /** Lowercase, whitespace-collapsed, punctuation-stripped form for exact matching. */
     ...
     * Valid rows are inserted (in one transaction); invalid or exact-duplicate
     * rows are reported with their line number so the admin can fix and
     * re-upload. Returns imported/total/errors (and 'warnings', which is
     * always empty after the removal of near-duplicate warn mode).
     */
    public static function import(array $rows, array $defaults, int $actorId): array
    {
        ...
        try {
            foreach ($rows as $i => $row) {
                ...
                try {
                    ...
                    // Duplicate gate (same layer as save()): an exact match
                    // rejects the row so no duplicate questions are imported.
                    $dups = self::duplicateCheck($n['question']);
                    $blocking = self::blockingHits($dups);
                    if ($blocking !== []) {
                        throw new InvalidArgumentException(self::duplicateMessage($blocking));
                    }
        ...
        return ['imported' => $inserted, 'total' => $total, 'errors' => $errors, 'warnings' => $warnings];
    }
