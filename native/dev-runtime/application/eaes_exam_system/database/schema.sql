...
-- ---------------------------------------------------------------------
-- students — identity registry (one row per roll_number + stream).
-- The exam portal is gated by ID (roll_number + stream) + password.
-- password_hash is NULL until the student registers (or claims a legacy
-- password-less row created by the old name/roll login), so a NULL hash
-- means the account cannot log in yet.
-- ---------------------------------------------------------------------
CREATE TABLE `students` (
  ...
  `password_hash` VARCHAR(255) NULL DEFAULT NULL COMMENT 'bcrypt hash; NULL until the student registers/claims an account',
  `last_login_at` DATETIME NULL DEFAULT NULL COMMENT 'Timestamp of the most recent successful student login',
  `deleted_at` DATETIME NULL DEFAULT NULL COMMENT 'Soft-delete marker: NULL = active, set = archived (restorable)',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
